...
// lets add a new route so the users can tell the server stuff about themselves
router.route('/hello/:user_name')
  ...
  .post((req, res) => {
    ...
    res.send(`Hi, ${ user_name }! I see your mouse at ${ req.body.left }, ${ req.body.top }`);
  });

// we should allow an admin to get information about all the users right?
router.route('/where')
  .get((req, res) => {
    // we should respond in json format because rule 1.
    res.json(users);
  });
